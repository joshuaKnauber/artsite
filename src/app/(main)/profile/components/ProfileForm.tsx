...
const ProfileForm = ({ metadata, invites, username }: ProfileFormPRops) => {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="mx-auto mt-8 flex w-full max-w-lg flex-col gap-6 px-4 md:mt-16"
    >
      <Link
        href={`/user/${username}`}
        className="text-light underline opacity-50"
      >
        View Profile
      </Link>
      <TextInput
      ...
      <div className="mt-4 w-fit rounded-md bg-white text-black transition-all md:hover:brightness-90">
        <button
          disabled={loading}
          type="submit"
